...
describe('mod routing', () => {
  ...
  it('provides a manual TEC Tracker for every rank value and resets the duel', async () => {
    ...
    expect(within(result).getByLabelText('S POW')).toBeTruthy()
    expect(result.classList.contains('tec-result-rank-s')).toBe(true)
    expect(result.classList.contains('tec-result-victory-pow')).toBe(true)
